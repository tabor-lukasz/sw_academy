#include "judge.h"
// Y            ou can't use #include - the only allowed is above
...
inline int distance(const Point p1, const Point p2) {
    auto a = (p1.x - p2.x) > 0 ? (p1.x - p2.x) : (p2.x - p1.x);
    auto b = (p1.y - p2.y) > 0 ? (p1.y - p2.y) : (p2.y - p1.y);
...
void slideDot(Point& src, Point& dest) {
    char tmp = board[src.y][src.x];
...
void moveDot(Point& src,const Point& dest) {
    while (src != dest) {
        int xDir = src.x < dest.x ? 1 : -1;
        int yDir = src.y < dest.y ? 1 : -1;
...
void tansportPiece(Point& dot, Point& piece, Point dest, bool reorder) {
    if (!reorder) {
        ...
        while (piece.y != dest.y) {
            int dir = piece.y < dest.y ? 1 : -1;
            if (dot.y > piece.y ) {
                slideDot(dot,piece);
            } else {
                int pass_side = dot.x < 5 ? 1 : -1;
                slideDot(dot,Point(dot.y,dot.x + pass_side));
                slideDot(dot,Point(dot.y + dir,dot.x));
                slideDot(dot,Point(dot.y + dir,dot.x));
                slideDot(dot,Point(dot.y,dot.x - pass_side));
            }
        }
